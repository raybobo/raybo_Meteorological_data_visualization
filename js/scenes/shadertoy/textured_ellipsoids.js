var textured_ellipsoids = [
  '// https://www.shadertoy.com/view/XsfXW8',
  '// Gardner textured ellipsoids -',
  '// https://www.cs.drexel.edu/~david/Classes/Papers/p297-gardner.pdf + bib',
  '',
  ...
  // '  return (texture(iChannel2, vec2((.5 + float(ascii)) / 256., 0.75)).x >
  // 0.);',
  // '}',
  '',
  ...
  '  vec3 OR = O / R,',
  '       DR = D / R;  // to space where ellipsoid is a sphere',
  ...
  '      nd = clamp(',
  '          -dot(Nn, D), 0., 1.);  // ratio of camera-facing (for silhouette)',
  '',
  '',
  '  float ns = fbm(P), ni = fbm(Pm + 10.);',
  '  float A, l0 = 3.;',
  '  // l += l*(l/l0-1.)/(1.+l*l/(l0*l0));     // optical depth modified at',
  '  // silhouette',
  ...
  '  z = -PI / 2. * cos(time);',
  '  vec3 O = vec3(',
  '      -15. * cos(t) * cos(z), 15. * sin(t) * cos(z), 15. * sin(z));  // camera',
...
export default textured_ellipsoids;
...
function init() {
  ...
  camera = new THREE.PerspectiveCamera(
      75, window.innerWidth / window.innerHeight, 1, 3000);
  ...
  gui.add(effectController, 'mouseSize', 1.0, 100.0, 1.0)
...
function initWater() {
  ...
  // material: make a ShaderMaterial clone of MeshPhongMaterial, with customized
  // vertex shader
  ...
  // Create a 4x1 pixel image and a render target (Uint8, 4 channels, 1 byte per
  // channel) to read water height and orientation
  ...
  for (var i = 0; i < NUM_SPHERES; i++) {
    ...
    if (sphere) {
      ...
      gpuCompute.doRenderTarget(
          readWaterLevelShader, readWaterLevelRenderTarget);
